...
export default function EmailVerification({ formData, setFormData, onEmailVerified }) {
    ...
    // 🔥 즉시 반영되지 않는 문제 해결: `useEffect`로 이메일 변경 감지 후 검증 실행
    useEffect(() => {
        if (formData.email) {
            validateEmail(formData.email);
        }
    }, [formData.email]); // formData.email이 변경될 때마다 실행
    ...
    const sendVerificationCode = () => {
        ...
        apiAxios
            .post("/sendVerificationCode", { email: formData.email })
            .then(() => {
    ...
    const verifyCode = () => {
        apiAxios
            .post("/verifyCode", { email: formData.email, verificationCode })
            .then((res) => {
